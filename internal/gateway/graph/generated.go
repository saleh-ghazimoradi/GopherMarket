...
func (ec *executionContext) unmarshalInputUpdateProductInput(ctx context.Context, obj any) (dto.UpdateProductRequest, error) {
	...
	for _, k := range fieldsInOrder {
		...
		case "category_id":
			ctx := graphql.WithPathContext(ctx, graphql.NewPathWithField("category_id"))
			data, err := ec.unmarshalNUInt2ᚖuint(ctx, v)
			if err != nil {
				return it, err
			}
			it.CategoryId = data
		...
		case "price":
			ctx := graphql.WithPathContext(ctx, graphql.NewPathWithField("price"))
			data, err := ec.unmarshalNFloat2ᚖfloat64(ctx, v)
			if err != nil {
				return it, err
			}
			it.Price = data
...
func (ec *executionContext) unmarshalNUInt2ᚖuint(ctx context.Context, v any) (*uint, error) {
	res, err := graphql.UnmarshalUint(v)
	return &res, graphql.ErrorOnPath(ctx, err)
}
